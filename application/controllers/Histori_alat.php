...
class Histori_alat extends CI_Controller
{
	...
	public function atasan()
	{
		...
		$data['title'] = 'Histori Alat Kerja';
		$this->load->view('templates/header', $data);
		$this->load->view('atasan/histori_alat_kerja/histori_alat_kerja', $data);
		$this->load->view('templates/footer');
	}

	public function proses_edit_status_disetujui_histori()
	{
		$status = ($this->input->post('status') == 'on') ? '1' : '0';
		$this->db->where('id_histori_alat', $this->input->post('id_histori_alat'));
		$result = $this->db->update('t_histori_alat', ['sudah_disetujui' => $status, 'id_atasan' => $this->session->userdata('id_personil')]);

		if ($result) {
			$this->session->set_flashdata('message', '<strong>Status Histori Alat Kerja Berhasil Disetujui</strong>
			<i class="bi bi-check-circle-fill"></i>');
		} else {
			$this->session->set_flashdata('message', '<strong>Status Histori Alat Kerja gagal Disetujui</strong>
	...
	public function proses_tambah_histori_alat_kerja()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if (!empty($signatureData)) {
				$data_histori_alat['tanda_tangan'] = $fileName;
			}
			...
			// echo "<pre>";
			// echo var_dump($data_histori_alat);
			// echo "<pre>";
