<div class="container-fluid py-0">
            ...
            <div class="card mb-4">
                <div class="row mb-4">
                    ...
                    <div class="col-6 pt-4 text-end">
                        <div class="mx-3">
                            <a href="<?= base_url() ?>atasan/jsa/tambah-jsa" class="btn bg-gradient-dark">+ Tambah Laporan JSA</a>
                        </div>
                    </div>
                </div>
                                ...
                                <?php foreach ($jsa as $j) : ?>
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <?php else : ?>
                                                <span style="cursor: pointer;" class="badge badge-sm bg-gradient-warning" data-bs-toggle="modal" data-bs-target="#edit_status_jsa<?= $j->id_jsa ?>">Menunggu Disetujui <i class=" bi bi-pencil-square"></i></span>
                                            <?php endif; ?>
                                        </td>

                                        <td class="text-start">
                                            <?php if ($j->id_personil == $this->session->userdata('id_personil')) : ?>
                                                <form action="<?= base_url('atasan/jsa/edit-jsa') ?>" method="post" class="d-inline-block">
    ...
    <?php foreach ($jsa as $jm) : ?>
        ...
        <div class="modal fade" id="edit_status_jsa<?= $jm->id_jsa ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Ubah status Laporan JSA</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    ...
                    <?php if ($check_signature->tanda_tangan) : ?>
                        <div class="modal-body">
                            <div class="form-group">
                                <form action="<?= base_url() ?>jsa/proses_edit_status_jsa" method="post">
                                    <label for="sudah_disetujui" class="form-control-label">Apakah Laporan JSA disetujui?</label>
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" name="sudah_disetujui" type="checkbox" id="flexSwitchCheckDefault" <?php echo ($jm->sudah_disetujui == '1') ? 'checked' : ''; ?>>
                                    </div>
                            </div>
                        </div>
                        <div class="modal-footer">
